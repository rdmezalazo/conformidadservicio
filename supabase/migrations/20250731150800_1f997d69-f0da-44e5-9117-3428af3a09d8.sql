-- Crear una solución alternativa: transferir registros a un usuario dummy antes de eliminar
DO $$
DECLARE
...
BEGIN
  ...
  IF FOUND THEN
    -- Buscar un usuario dummy/admin existente para transferir los registros
    SELECT id INTO dummy_user_id
    FROM auth.users
    WHERE email LIKE '%admin%' OR email LIKE '%dummy%'
    AND deleted_at IS NULL
    ORDER BY created_at DESC
    LIMIT 1;
    
    -- Si no hay usuario admin/dummy, usar el primer usuario disponible
    IF NOT FOUND THEN
      SELECT id INTO dummy_user_id
      FROM auth.users
    ...
    IF FOUND THEN
      -- Transferir conformidades_servicio al usuario dummy
      UPDATE public.conformidades_servicio 
      SET usuario_id = dummy_user_id
      WHERE usuario_id = user_id_to_delete;
      
      RAISE NOTICE 'Registros transferidos al usuario con ID: %', dummy_user_id;
    ELSE
      -- Si no hay otros usuarios, hacer la columna nullable temporalmente
      ...
      RAISE NOTICE 'Registros desasociados (usuario_id set to NULL)';
    END IF;
    
    -- Eliminar registros relacionados en tablas públicas
